...
def my_cvs():
    ...
    for cv in cvs:
        pdf_path = os.path.join(
            app.config["UPLOAD_FOLDER"], "gen_cvs", f"cv_{cv.id}.pdf"
        )
        ...
        cv_list.append(
            {
                ...
                "pdf_url": url_for("download_cv_pdf", cv_id=cv.id)
                if os.path.exists(pdf_path)
                else None,
...
def download_cv_pdf(cv_id):
    ...
    if os.path.exists(pdf_path):
        return send_from_directory(
            os.path.join(app.config["UPLOAD_FOLDER"], "gen_cvs"), f"cv_{cv_id}.pdf"
        )
    else:
...
def generate_pdf():
    ...
    if not wkhtmltopdf_path:
        try:
            ...
            path = (
                subprocess.check_output(["where", "wkhtmltopdf"], shell=True)
                .decode("utf-8")
                .strip()
            )
    ...
    style_name = style_map.get(
        processed_data.get("template", "modern").lower(), "Modern"
    )
    ...
    for i in range(len(work_titles)):
        we = WorkExperience(
            ...
            start_date=parse_date(work_start_dates[i])
            if i < len(work_start_dates)
            else None,
            end_date=parse_date(work_end_dates[i])
            if i < len(work_end_dates) and work_end_dates[i]
            else None,
    ...
    for i in range(len(edu_degrees)):
        edu = Education(
            ...
            start_date=parse_date(edu_start_dates[i])
            if i < len(edu_start_dates)
            else None,
            end_date=parse_date(edu_end_dates[i])
            if i < len(edu_end_dates) and edu_end_dates[i]
            else None,
    ...
    try:
        pdf_data = pdfkit.from_file(
            html_file_path, False, options=options, configuration=config
        )
        ...
        error_message += (
            f"<p>The application looked for wkhtmltopdf at: {wkhtmltopdf_path}</p>"
        )
        error_message += (
            f"<p>If wkhtmltopdf is installed elsewhere, please check the path.</p>"
        )
